...
export const apiScriptList = async (page?: number, page_size?: number, tag?: string
  ...
  const response = await http.get<IResponse<ResScriptList[]>>("/common/cs/list", { params })
  return response.data
}
...
export const apiScriptDetail = async (cs_id: number) => {
  const resposne = await http.get<IResponse<ResScriptDetail>>(`/common/cs/${cs_id}/detail`)
...
export const apiScriptDebugByID = async (cs_id: number) => {
  const response = await http.get<IResponse<ResDebugByID>>(`/common/cs/${cs_id}/debug`)
  return response.data
}

// 通过ACE编辑器写的代码调试
export const apiScriptDebugByTxt = async (data: ReqPyScript) => {
  const response = await http.post<IResponse<ResDebugByID>>("/script/execute_python", data)
  return response.data
}
